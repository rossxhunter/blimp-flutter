...
class Suggestions {
  ...
  Future<void> getSuggestions() async {
    try {
      var suggestionsResponse = await makeGetRequest("suggestions", "");
      Map suggestions = json.decode(suggestionsResponse);
      destinationSuggestions = suggestions["destinations"];
      activitySuggestions = suggestions["activities"];
      currencySuggestions = suggestions["currencies"];
      exploreSuggestions = suggestions["explore"];
      attractionsSuggestions = suggestions["attractions"];
      searchSuggestions = suggestions["search"];
      availableFlights = suggestions["availableFlights"];
      testingSuggestions = suggestions["testing"];
      countrySuggestions = suggestions["countries"];
  ...
  String getActivityFromId(String id) {
    return activitySuggestions.where((a) => a["id"] == id).toList()[0]
        ["plural"];
  }
